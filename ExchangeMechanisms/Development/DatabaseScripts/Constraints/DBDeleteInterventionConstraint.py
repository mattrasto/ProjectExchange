#-------------------------------------------------------------------------------
# Name:        DBDeleteInterventionConstraint
# Version:     3.0
...
def main(ContractNumber, ConstraintID):
    ...
    try:
        print ""
        cursor.execute("""SELECT * FROM InterventionConstraintBook WHERE ConstraintID = "%s" """ % (ConstraintID))
    ...
    #Deletes constraint from InterventionConstraintBook
    try:
        cursor.execute("""DELETE FROM InterventionConstraintBook WHERE ConstraintID = "%s" """ % (ConstraintID))
        db.commit()
        print ""
    ...
    if ConstraintType == "MTC":
        try:
            cursor.execute("""UPDATE MTCBook SET UserInterventionConstraints = (UserInterventionConstraints - 1) WHERE MTCNumber = %d""" % (ContractNumber))
            db.commit()
            print ""
            print "User Intervention Constraints Successfully Updated"
        except:
    ...
    if ConstraintType == "Loan":
        try:
            cursor.execute("""UPDATE LoanBook SET UserInterventionConstraints = (UserInterventionConstraints - 1) WHERE ContractNumber = %d""" % (ContractNumber))
            db.commit()
            print ""
            print "User Intervention Constraints Successfully Updated"
        except:
    ...
    if ConstraintType == "Private Trade":
        try:
            cursor.execute("""UPDATE PrivateTradeBook SET UserInterventionConstraints = (UserInterventionConstraints - 1) WHERE TradeNumber = %d""" % (ContractNumber))
            db.commit()
            print ""
            print "User Intervention Constraints Successfully Updated"
        except:
    ...
    Comment = "Deleted User Intervention Constraint"
    ...
    try:
        print ""
        cursor.execute("""INSERT INTO ControlLog(Employee, Action, AffectedRows, AffectedAttributes, Comment) VALUES("%s", "%s", "%s", "%s", "%s")""" % (Employee, "Delete User Intervention Constraint", ConstraintID, "All", Comment))
        db.commit()
