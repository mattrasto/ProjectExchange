...
# Version:     3.0
# Purpose:     Searches MTCBook with specified constraints
#
# Author:      Matthew
#
# Created:     05/26/2014
...
def MTCPrint(MTCs):
    ...
    for MTC in MTCs:
        ...
        print "Dividend Type: " + str(MTC[10])
        print "Minimum Borrower Constraints: " + str(MTC[11])
        print "User Intervention Constraints: " + str(MTC[12])
        print "User Requests: " + str(MTC[13])
        print "Date Entered: " + str(MTC[14])
        print ""
    print "------------------------------"
...
def main(SearchParameter, SearchValue):
    ...
    if SearchParameter == "INTEREST COMPOUND RATE":
        ...
        InterestCompoundRateSearch = """SELECT * FROM MTCBook WHERE (YEAR(InterestCompoundRate) LIKE "%s") AND (MONTH(InterestCompoundRate) LIKE "%s") AND (DAY(InterestCompoundRate) LIKE "%s") AND (HOUR(InterestCompoundRate) LIKE "%s") AND (MINUTE(InterestCompoundRate) LIKE "%s") AND (SECOND(InterestCompoundRate) LIKE "%s")""" % (YearParameter, MonthParameter, DayParameter, HourParameter, MinuteParameter, SecondParameter)
        try:
    ...
    if SearchParameter == "DURATION":
        ...
        #Uses "LIKE" operator to compare value with wildcards, which replaces value with an "ANY" search in MTC to search for current value if input was left blank
        DurationSearch = """SELECT * FROM MTCBook WHERE (YEAR(Duration) LIKE "%s") AND (MONTH(Duration) LIKE "%s") AND (DAY(Duration) LIKE "%s") AND (HOUR(Duration) LIKE "%s") AND (MINUTE(Duration) LIKE "%s") AND (SECOND(Duration) LIKE "%s")""" % (YearParameter, MonthParameter, DayParameter, HourParameter, MinuteParameter, SecondParameter)
        try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        for Field in FieldList:
            ...
            if TargetParameter.upper() == SearchParameter or SearchParameter == "MTC NUMBER" or SearchParameter == "DATE ENTERED" or SearchParameter == "INTEREST COMPOUND RATE" or SearchParameter == "INTEREST RATE" or SearchParameter == "STOP LOSS PRICE" or SearchParameter == "FULFILLMENT PRICE" or SearchParameter == "DIVIDEND TYPE" or SearchParameter == "MINIMUM BORROWER CONSTRAINTS" or SearchParameter == "USER INTERVENTION CONSTRAINTS" or SearchParameter == "USER REQUESTS":
                break;
        
        #Retrying until valid input is given
        while TargetParameter.upper() != SearchParameter and SearchParameter != "MTC NUMBER" and SearchParameter != "DATE ENTERED" and SearchParameter != "INTEREST COMPOUND RATE" and SearchParameter != "INTEREST RATE" and SearchParameter != "STOP LOSS PRICE" and SearchParameter != "FULFILLMENT PRICE" and SearchParameter != "DIVIDEND TYPE" and SearchParameter != "MINIMUM BORROWER CONSTRAINTS" and SearchParameter != "USER INTERVENTION CONSTRAINTS" and SearchParameter != "USER REQUESTS":
    ...
    if SearchParameter != "DATE ENTERED" and SearchParameter != "INTEREST COMPOUND RATE" and SearchParameter != "DURATION":
        ...
        while SearchValue == "":
            ...
            SearchValue = raw_input("Search for value: ")
    #Requests date input and testing values for validity
    else:
