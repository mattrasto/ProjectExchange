...
cursor.execute("""INSERT INTO LoanBook(Username, Medium, Volume, Action, InterestCompoundRate, InterestRate, Duration, EndPoint, DividendType) VALUES("%s", "%s", %f, "%s", "%s", %f, "%s", "%s", "%s")""" % (Username, ContractMedium.title(), Volume, OrderAction.title(), InterestCompoundRate, InterestRate, Duration, FormattedDurationDateTime, DividendType.title()))
db.commit()
print ""
    ...
    if BorrowerConstraintBookConstraints == ():
        ...
        for Constraint in BorrowerConstraintBookConstraints:
            ...
            if int(ConstraintNumber) > int(OldConstraintNumber):
                #print "Constraint Number: " + str(ConstraintNumber)
                #print "Old Constraint Number: " + str(OldConstraintNumber)
                ConstraintID = (str(LoanNumber) + "-" + str(int(ConstraintNumber) + 1))
...
try:
    print ""
    cursor.execute("""INSERT INTO LoanLog(ContractNumber, Username, Medium, Volume, Action, InterestCompoundRate, InterestRate, Duration, EndPoint, DividendType) VALUES(%d, "%s", "%s", %f, "%s", "%s", %f, "%s", "%s", "%s")""" % (ContractNumber, Username, ContractMedium.title(), Volume, OrderAction.title(), InterestCompoundRate, InterestRate, Duration, FormattedDurationDateTime, DividendType.title()))
    db.commit()
