#-------------------------------------------------------------------------------
# Name:        DBDeleteBorrowerConstraintTest
# Version:     2.0
# Purpose:     Load and Remote Testing for DBDeleteBorrowerConstraint
#
# Author:      Matthew
#
# Created:     08/19/2014
...
#Imports main module
import DatabaseScripts.Constraints.DBDeleteBorrowerConstraint as Mod
...
if Option == 2:
    ...
    cursor.execute("""SELECT ConstraintID FROM BorrowerConstraintBook ORDER BY RAND() LIMIT %d """ % (OptionTwoLoops))
    ...
    if ConstraintIDs != ():
        for Index, ConstraintID in enumerate(ConstraintIDs):
            ...
            print "----------Starting Round: " + str(Index + 1) + "----------"
            print ""
            cursor.execute("""SELECT ContractNumber FROM BorrowerConstraintBook WHERE ConstraintID = "%s" """ % (ConstraintID))
